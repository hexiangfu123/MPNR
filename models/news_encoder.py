...
class NewsEncoder(nn.Module):
    def __init__(self, path, config, apply_reduce_dim: bool, use_sapo: bool, dropout: float,
        ...
        # self.embed_dim = self.bert.embed_dim
    ...
    def forward(self, title_encoding: Tensor, title_attn_mask: Tensor, sapo_encoding: Union[Tensor, None] = None,
        ...
        news_info = []
        # Title encoder
        title_repr = self.bert(input_ids=title_encoding, attention_mask=title_attn_mask)[1]
        # title_repr = title_repr_s[0][:, 0, :]
        # title_pool_helper = title_repr_s[1]
        ...
        # Sapo encoder
        if self.use_sapo:
            sapo_repr = self.bert(input_ids=sapo_encoding, attention_mask=sapo_attn_mask)[1]
            # sapo_repr = sapo_repr[:, 0, :]
            if self.apply_reduce_dim:
